...
if not os.path.exists(os.path.join(index_dir, 'index_test.npy')):
    bow_descs_test, img_paths_test = create_index(
        test_folders, vocabulary, crossCheck=False)
    np.save(os.path.join(index_dir, 'index_test.npy'), bow_descs_test)
    with open(os.path.join(index_dir, 'index_paths_test.txt'), mode='w+') as file:
        json.dump(img_paths_test, file)
    print('Test Index created')
else:
    bow_descs_test = np.load(os.path.join(index_dir, 'index_test.npy'))
    with open(os.path.join(index_dir, 'index_paths_test.txt'), mode='r') as file:
