...
if not any([os.path.exists(index_filename), os.path.exists(index_paths_filename)]):
    ...
    bow_descs_trained, img_paths_train = create_index(train_folders, vocabulary)
...
if not os.path.exists(os.path.join(root_dir, 'index_test.npy')):
    bow_descs_test, img_paths_test = create_index(test_folders, vocabulary, crossCheck=False)
